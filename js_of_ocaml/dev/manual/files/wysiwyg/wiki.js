...
b4=125,cB=254,ac=255,cs=224,ay='"',z=16777215,b3="\\\\",cr=250,ae=1024,cd="jsError",a2=2147483,b2="input",a8=115,cA=512,au="formatblock",e="",d=128,cq="^",cp=100,y="0",cz="</a>",cx="p",cy="fd ",aB="inserthtml",co="[[",cn=1e3,aA="src/core/lwt.ml",a4="main.ml",az=".",a3="+",ad=65535,b1="\n\n",af=105,cc="%d",cw=-88,a7=110,cb=57343,cm=785140586,b0=252,ax="'",cl=127,aw="int_of_string",ck=-32,ca=982028505,bZ="cols",G=" ",a6="e",cj="I",cv="h1",b$="h3",cu="]]",T="-",av=-48,ci="nan",b_='<a href="',b8=240,b9=2048,cg="%.12g",ch=56320,a5=" : file already exists",b7="rows",cf="/",bY="5px",ce=114,b6="h2",ct="index out of bounds",b5="B";function
bh(a,b,c){var
e=new
Array(c);for(var
...
f=e;for(;0<c;b+=ae,c-=ae)f+=d.apply(null,bh(a,b,Math.min(c,ae)));return f}function
aC(a){var
c=new
Array(a.l),e=a.c,d=e.length,b=0;for(;b<d;b++)c[b]=e.charCodeAt(b);for(d=a.l;b<d;b++)c[b]=0;a.c=c;a.t=4;return c}function
A(a,b,c,d,e){if(e==0)return 0;if(d==0&&(e>=c.l||c.t==2&&e>=c.c.length)){c.c=a.t==4?bg(a.c,b,e):b==0&&a.c.length==e?a.c:a.c.substr(b,e);c.t=c.c.length==c.l?0:2}else
...
if(e<0)return D(c.apply(null,bh(b,0,a)),bh(b,a,d-a));else
...
a$(a,b){var
c,f=bd(a),e=f.prec<0?6:f.prec;if(b<0){f.sign=-1;b=-b}if(isNaN(b)){c=ci;f.filler=G}else
...
fB(a,b){return+(cC(a,b,false)>=0)}function
...
fL(a,b){var
e=0,d=cN(a),c=cN(b),f=[ac,0,0,0];while(cE(d,c)>0){e++;cD(c)}while(e>=0){e--;cD(f);if(cE(d,c)>=0){f[1]++;d=fJ(d,c)}fG(c)}return[0,f,d]}function
fK(a){return a[1]|a[2]<<24}function
fE(a){return a[3]<<16<0}function
fH(a){var
b=-a[1],c=-a[2]+(b>>24),d=-a[3]+(c>>24);return[ac,b&z,c&z,d&ad]}function
fD(a,b){var
c=bd(a);if(c.signedconv&&fE(b)){c.sign=-1;b=fH(b)}var
...
if(b>=56319||f+1==i||(j=a.charCodeAt(f+1))<ch||j>cb)c+="\xef\xbf\xbd";else{f++;b=(b<<10)+j-56613888;c+=String.fromCharCode(b8|b>>18,d|b>>12&63,d|b>>6&63,d|b&63)}if(c.length>ae){c.substr(0,1);h+=c;c=e}}return h+c}function
E(a){var
b=9;if(!cF(a))b=8,a=f8(a);return new
p(b,a,a.length)}function
ga(a){var
d=a.length,c=new
Array(d);for(var
b=0;b<d;b++)c[b]=a[b];return c}function
cH(a){return function(){return arguments.length>0?D(a,ga(arguments)):D(a,[undefined])}}function
Q(a,b){var
a=a+1|0,c=new
Array(a);c[0]=0;for(var
d=1;d<a;d++)c[d]=b;return c}function
r(a){be(f[2],a)}function
bc(a){if(!a.opened)r("Cannot flush a closed channel");if(a.buffer==e)return 0;if(a.output)switch(a.output.length){case
...
e=f.toString(),g=e.lastIndexOf("\n");if(g<0)a.buffer+=e;else{a.buffer+=e.substr(0,g+1);bc(a);a.buffer+=e.substr(g+1)}return 0}function
cK(a,b){var
...
fS(a,b){return+(cC(a,b,false)!=0)}function
fT(a){return+(a
instanceof
Array)}function
cO(a){return a
...
bf(a,b,c){if(b>>>0>=a.l)cR();return l(a,b,c)}function
...
instanceof
b.Error)return[0,fR(cd),a];return[0,f[3],E(String(a))]}function
...
cW=c(cg),cV=c(az),cT=c("true"),cU=c("false"),c0=c("\\b"),c1=c("\\t"),c2=c("\\n"),c3=c("\\r"),cZ=c(b3),cY=c("\\'"),c6=c(e),c5=c("String.blit"),c4=c("String.sub"),c7=c("Queue.Empty"),c9=c("Buffer.add: cannot grow buffer"),dp=c(e),dq=c(e),dt=c(cg),du=c(ay),dv=c(ay),dr=c(ax),ds=c(ax),dn=c(ci),dl=c("neg_infinity"),dm=c("infinity"),dk=c(az),dj=c("printf: bad positional specification (0)."),di=c("%_"),dh=[0,c("printf.ml"),143,8],de=c(ax),df=c("Printf: premature end of format string '"),da=c(ax),db=c(" in format string '"),dc=c(", at char number "),dd=c("Printf: bad conversion %"),c_=c("Sformat.index_of_int: negative argument "),dA=c(e),dB=c(", %s%s"),dS=[1,1],dT=c("%s\n"),dU=c("(Program not linked with -g, cannot print stack backtrace)\n"),dM=c("Raised at"),dP=c("Re-raised at"),dQ=c("Raised by primitive operation at"),dR=c("Called from"),dN=c('%s file "%s", line %d, characters %d-%d'),dO=c("%s unknown location"),dH=c("Out of memory"),dI=c("Stack overflow"),dJ=c("Pattern matching failed"),dK=c("Assertion failed"),dL=c("Undefined recursive module"),dD=c("(%s%s)"),dE=c(e),dF=c(e),dG=c("(%s)"),dz=c(cc),dx=c("%S"),dy=c("_"),d6=[0,c(aA),648,20],d7=[0,c(aA),651,8],d3=[0,c(aA),498,8],d1=[0,c(aA),487,9],dZ=c("Lwt.wakeup_result"),dW=c("Fatal error: exception "),dV=c("Lwt.Canceled"),ea=c("Js.Error"),ee=c("iframe"),ed=c("br"),ec=c("textarea"),eb=c(b2),ei=c("Exception during Lwt.async: "),eR=c("^error_in_anchor^"),eG=[0,c(a4),72,76],eH=c("global"),eI=c("wiki"),eJ=c("^error2_in_anchor^"),eK=[0,c(cu),0],eL=c(co),eM=c(e),eN=[0,c(cu),0],eO=c("|"),eP=c(co),eQ=c(e),eF=[0,c(a4),67,35],em=c("#text"),en=c("A"),eo=c(b5),ep=c("BR"),eq=c("DIV"),er=c("H1"),es=c("H2"),et=c("H3"),eu=c("HR"),ev=c(cj),ew=c("P"),ex=c(cq),ey=c(cq),ez=c(b1),eA=[0,c("//")],eB=c("----"),eD=c(b3),eE=[0,c("**")],eC=c(b1),el=c(e),fl=c("lololo"),fm=c("Enter a wikipage"),fn=c(e),fo=[0,c(cz),0],fp=c('" wysitype="wiki">'),fq=c(b_),fb=c("http://google.ru"),fc=c("Enter a link"),fd=c("desc"),fe=c("Enter description"),ff=[0,c(cz),0],fg=c('" wysitype="global">'),fh=c(b_),fi=c(e),eT=c("inserthorizontalrule"),eU=c("hr"),eV=c("removeformat"),eW=c("remove format"),eX=c("bold"),eY=c(b5),eZ=c("italic"),e0=c(cj),e1=c(au),e2=c(cx),e3=[0,[0,c(cx)]],e4=c(au),e5=c(cv),e6=[0,[0,c(cv)]],e7=c(au),e8=c(b6),e9=[0,[0,c(b6)]],e_=c(au),e$=c(b$),fa=[0,[0,c(b$)]],fj=c(aB),fk=c("link"),fr=c(aB),fs=c("link2wiki"),ft=c(e),eS=[0,c(a4),96,17],ej=c("Main.Break");function
...
c=[0,h(a)];bn(function(a){A(d,0,e,c[1],h(d));c[1]=c[1]+h(d)|0;A(a,0,e,c[1],h(a));c[1]=c[1]+h(a)|0;return 0},i);return e}return c6}var
...
Z(a,b){var
c=a[2];if(a[3]<=c)bo(a,1);bf(a[1],c,b);a[2]=c+1|0;return 0}function
q(a,b){var
c=h(b),d=a[2]+c|0;if(a[3]<d)bo(a,c);ai(b,0,a[1],a[2],c);a[2]=d;return 0}function
aQ(a){return 0<=a?a:aM(k(c_,aO(a)))}function
bp(a,b){return aQ(a+b|0)}var
c$=1;function
bq(a){return bp(c$,a)}function
br(a){return ah(a,0,h(a))}function
bs(a,b,c){var
d=k(db,k(a,da)),e=k(dc,k(aO(b),d));return V(k(dd,k(X(1,c),e)))}function
_(a,b,c){return bs(br(a),b,c)}function
am(a){return V(k(df,k(br(a),de)))}function
dg(d){function
a(a,b){var
...
bt(a,b,c,d,e){var
f=I(b,c,d,e);if(78!==a)if(a7!==a)return f;bf(f,h(f)-1|0,117);return f}function
bu(a){return function(d,b){var
j=h(d);function
k(a,b){var
m=40===a?41:b4,c=b;for(;;){if(j<=c)return am(d);if(37===w(d,c)){var
...
aa=p(g,f);if(0===c)var
aw=dt;else{var
Y=I(n,r,a,c);if(70===u)bf(Y,h(Y)-1|0,103);var
aw=Y}var
...
P=ak(h(S)),ao=function(a,b){Z(P,b);return a+1|0};bv(S,function(a,b,c){if(a)q(P,di);else
...
b(a){return d}var
c=0;return function(a){return bz(c,b,cX,bj,bm,e,a)}}function
dw(a){return ak(2*h(a)|0)}function
v(a){function
b(a){var
b=al(a);a[2]=0;return b}return bz(1,dw,Z,q,function(a){return 0},b,a)}var
aR=[0,0];function
aT(a,b){var
c=a[b+1];if(fT(c)){if(cO(c)===b0)return i(v(dx),c);if(cO(c)===253){var
...
bB(a,b){if(a.length-1<=b)return dA;var
c=bB(a,b+1|0),d=aT(a,b);return s(v(dB),d,c)}function
dC(a){var
b=a.length-1;if(2<b>>>0){var
c=bB(a,2),d=aT(a,1);return s(v(dD),d,c)}switch(b){case
0:return dE;case
...
c=m;continue}if(b[1]===bD)return dH;if(b[1]===bE)return dI;if(b[1]===bF){var
e=b[2],h=e[3],p=e[2],q=e[1];return at(v(aS),q,p,h,h+5|0,dJ)}if(b[1]===u){var
f=b[2],j=f[3],r=f[2],s=f[1];return at(v(aS),s,r,j,j+6|0,dK)}if(b[1]===bG){var
...
var
g=i(v(dO),e);s(bA(a),dT,g)}var
q=b+1|0;if(f!==b){var
b=q;continue}break}}return 0}return i(bA(a),dU)}function
...
$(a){return aV(a)}var
bK=[0,function(a){bk(dW);bk(bC(a));cK(W,10);bH(W);bm(W);bl(0);return f2(2)}];function
bL(a,b){try{var
...
switch(e[0]){case
0:var
g=e[2];J[1]=e[1];bL(g,0);return a<50?N(1+a,d):F(N,[0,d]);case
1:var
...
d8=[0,function(a){return 0}],x=bJ(0),d9=[0,0],K=b,aa=null;function
d_(a){var
e=1-(x[2]===x?1:0);if(e){var
b=bJ(0);b[1][2]=x[2];x[2][1]=b[1];b[1]=x[1];x[1][2]=b;x[1]=x;x[2]=x;d9[1]=0;var
c=b[2];for(;;){var
...
ab=true,ap=false,bQ=K.Array,bR=[0,ea];fW(c(cd),[0,bR,{}][0+1]);bI(function(a){return a[1]===bR?[0,E(a[2].toString())]:0});bI(function(a){return a
instanceof
bQ?0:[0,E(a.toString())]});function
S(a){return a}function
L(a,b){a.appendChild(b);return 0}function
aq(d){return S(function(a){if(1-(S(a)==aa?1:0)){var
...
f=new
bQ();f.push("<",d.toString());ar(a,function(a){f.push(' type="',cG(a),ay);return 0});ar(b,function(a){f.push(' name="',cG(a),ay);return 0});f.push(">");return c.createElement(f.join(e))}var
g=aY(c,d);ar(a,function(a){return g.type=a});ar(b,function(a){return g.name=a});return g}}function
bV(a,b,c){return bU(a,b,c,ec)}function
aZ(a){return bS(a,ed)}K.HTMLElement===d$;var
ef=fO(0),eg=a2;function
eh(a){var
c=[0,[2,[0,1,0,0,0]]],g=[0,0];function
h(a,b){var
d=a2<a?[0,eg,a-a2]:[0,a,0],e=d[2],f=d[1],i=e==0?function(a){return bM(c,a)}:function(a){return h(e,a)};g[1]=[0,K.setTimeout(cH(i),f*cn)];return 0}h(a,0);function
i(a){var
...
j=1;break;default:var
j=0}return c}d8[1]=function(a){return 1===a?(K.setTimeout(cH(d_),0),0):0};function
bW(a){return ef.log(a.toString())}bK[1]=function(a){bW(ei);bW(bC(a));return bH(W)};function
...
j=function(a){return q(c,a)};return as(aj(eM,[0,eL,[0,d,eK]]),j)}return q(c,aj(eQ,[0,eP,[0,d,[0,eO,[0,M(0,b),eN]]]]))},m=function(a){return q(c,eR)};return bP(f.getAttribute("wysitype"),m,l)}var
n=b.nodeValue;function
h(a){return[0,a]}var
d=bP(n,function(a){return 0},h);return d?q(c,E(d[1])):0};bO(f.item(d),j);var
l=d+1|0;if(h!==d){var
d=l;continue}break}}return al(c)}K.onload=aq(function(a){function
d(a){throw[0,u,eS]}var
f=ao(B.getElementById("wiki_demo"),d),b=bS(B,ee);b.style.border="2px green solid";b.src="#";b.id="wysiFrame";L(f,b);function
...
a(a,b,c,d){var
j=a?a[1]:ap,h=b?b[1]:0,g=bU([0,"submit"],0,B,eb);g.value=c.toString();g.onclick=aq(function(a){i.focus();var
...
j=0===f?0.5:0.1;return d5(eh(j),i)}k(ft,0);return 0}bO(b.contentDocument,e);return ap});bl(0);return}(function(){return this}()));
